...
export interface BlobStorageClient {
   ...
   * @param {string} customerId: The customer identifier, this allows the implementation to make decisions using namespace
   restrictions by customer.
   * @param {string} itemType: The item type, this allows the implementation to make decisions for certain types of item
   * @returns {any}
   */
  setItem: (key: string, value: string, customerId: string, itemType: string) => any;

  /**
   * Takes a key and optional type. Deletes the matching item from the blob store
   * @param {string} key: The identifier for the item
   * @param {string} customerId: The item type, this allows the implementation to make decisions for certain types of item
